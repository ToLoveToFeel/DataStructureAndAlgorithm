        ...
        public void paintComponent(Graphics g){  // 我们不需要知道该参数g是如何得到的，g称为上下文环境
            ...
            for(int i = 0 ; i < data.N() ; i ++)
                for(int j = 0 ; j < data.M() ; j ++){
                    ...
                    if(data.open[i][j]){  // 如果已经打开了
                        if(data.isMine(i, j))  // 是雷的话放置雷
                            AlgoVisHelper.putImage(g2d, j*w, i*h, MineSweepData.mineImageURL);
                        else  // 不是的话放置对应的文字图片
                            AlgoVisHelper.putImage(g2d, j*w, i*h, MineSweepData.numberImageURL(data.getNumber(i, j)));
                    }
                    else{  // 如果还没有打开
                        if(data.flags[i][j])  // 插旗则显示旗
                            AlgoVisHelper.putImage(g2d, j*w, i*h, MineSweepData.flagImageURL);
                        else  // 没插旗则显示背景
                            AlgoVisHelper.putImage(g2d, j*w, i*h, MineSweepData.blockImageURL);
                    }
                }
