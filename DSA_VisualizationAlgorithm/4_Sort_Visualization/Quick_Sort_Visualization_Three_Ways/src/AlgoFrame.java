    ...
    private class AlgoCanvas extends JPanel{  // 有关画布的类
        ...
        public void paintComponent(Graphics g){  // 我们不需要知道该参数g是如何得到的，g称为上下文环境
            ...
            for (int i = 0; i < data.N(); i++){
                ...
                if (i == data.curPivot)    // 标定点元素为黄色
                    AlgoVisHelper.setColor(g2d, AlgoVisHelper.Yellow);

                if (i >= data.l + 1 && i <= data.lt)  // [data.l+1, data.lt]元素为浅蓝
                    AlgoVisHelper.setColor(g2d, AlgoVisHelper.LightBlue);
                if (i >= data.lt + 1 && i < data.curElement)// [data.lt+1, data.curElement]元素为橙色
                    AlgoVisHelper.setColor(g2d, AlgoVisHelper.Orange);
                if (i >= data.gt && i <= data.r)  // [data.gt, data.r]元素为浅蓝
                    AlgoVisHelper.setColor(g2d, AlgoVisHelper.LightBlue);

                if (i == data.curElement)  // 当前考察的元素为黑色
                    AlgoVisHelper.setColor(g2d, AlgoVisHelper.Black);
                if (data.fixedPivots[i])  // 已经确定位置的元素为红色
